...
static dispatch_queue_t mm_logQueue(void) {
    ...
    dispatch_once(&once, ^{
        queue = dispatch_queue_create("com.pkgfiles.mobilemeadowprefs.log", DISPATCH_QUEUE_SERIAL);
    });
    return queue;
}
...
void RLogv(NSString *fmt, va_list args) {
    ...
    NSString *logLine = [NSString stringWithFormat:@"[MobileMeadowPrefs] %@", message];
    NSLog(@"%@", logLine);
